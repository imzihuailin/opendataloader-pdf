...
本地模式在 CPU 上每秒处理 60 页以上（0.02s/page）。Hybrid mode 每秒处License理 2 页以上（0.46s/page），在复杂文档上准确率显著更高。无需 GPU。基准测试运行于 Apple M4。[完整基准测试详情](https://github.com/opendataloader-project/opendataloader-bench)。使用多进程批处理时，在 8+ 核机器上吞吐量每秒超过 100 页。
